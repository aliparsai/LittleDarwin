  ...
  public int problemInParadise(int rekTiefe) {
    ...
    def test_parseIssue13(self):
        self.assertRaises(ParseCancellationException,
                          self.javaParse.parse, self.issue13Code)

    def test_largeJavaFile(self):
        LargeJavaFile = bz2.decompress(base64.decodebytes(
            self.largeJavaFileBase64Bzipped)).decode('utf-8')
    ...
    def test_getMethodNameForNode(self):
        ...
        self.assertEqual("***not in a method***",
                         self.javaParse.getMethodNameForNode(parsedTree, 3))

    def test_getCyclomaticComplexity(self):
        parsedTree = self.javaParse.parse(self.factorialSourceCode)
        cyclomaticComplexityDict = self.javaParse.getCyclomaticComplexityAllMethods(
            parsedTree)
