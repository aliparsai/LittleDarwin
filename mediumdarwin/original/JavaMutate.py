...
from mediumdarwin.original.JavaParser import JavaParser
...
class Mutant(object):
        ...
        for mutation in self.mutationList:
            ...
            for pos in sorted(byteOffsetDict.keys()):
                if pos < mutation.startPos:
                    byteOffsetDict[mutation.startPos] = byteOffsetDict[pos] + \
                        mutation.byteOffset
    ...
    def stub(self) -> str:
        ...
        textStub = "/* LittleDarwin generated order-{0} mutant\n".format(
            str(len(self.mutationList)))  # type: str
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Mutant):
            if self.sourceCode == other.sourceCode:
                ...
                newMutant = Mutant(-1 * self.mutantID * other.mutantID,
                                   newMutationList, self.sourceCode)
    ...
    def generateMutants(self):
        ...
        for node, nodeType in self.mutableNodesWithTypes:
            ...
            for replacementText in replacementTextList:
                ...
                mutant = Mutant(mutantID=id, mutationList=[
    ...
    def filterCriteria(self):
        ...
        def isInInnerClass(javaParseObject, methodBody, variableName):
            methodNodes = javaParseObject.seekAllNodes(
                methodBody, JavaParser.MethodBodyContext)
        ...
        for methodDeclaration in self.allNodes:
            ...
            for variablesPerNode in variableList:
                assert isinstance(variablesPerNode,
                                  JavaParser.VariableDeclaratorIdContext)
        ...
        for node in self.mutableNodes:
            ...
            mutation = Mutation(startPos=node.children[self.terminalChild[node]].symbol.start,
                                endPos=node.children[self.terminalChild[node]
                                                     ].symbol.stop, lineNumber=node.start.line,
                                nodeID=node.nodeIndex, mutatorType=self.mutatorType, replacementText=replacementText,
                                color=self.color)
...
class JavaMutate(object):
        ...
        for mO in self.mutationOperators:
            ...
            for mutableNode in mO.mutableNodes:
                try:
                    ...
                    lineNumber = -1
                self.mutantsPerLine[lineNumber] = 1 + \
                    self.mutantsPerLine.get(lineNumber, 0)
                methodName = self.javaParseObject.getMethodNameForNode(
                    self.sourceTree, mutableNode.nodeIndex)
                self.mutantsPerMethod[methodName] = 1 + \
                    self.mutantsPerMethod.get(methodName, 0)
    ...
    def gatherHigherOrderMutants(self, higherOrderDirective: int, metaTypes: List[str] = ["Traditional"]):
        ...
        higherOrder = max(int(log10(len(selectedMutants)))
                          if higherOrderDirective == -1 else higherOrderDirective, 1)
        ...
        while len(selectedMutants) > 0:
            ...
            for mutation in higherOrderMutant.mutationList:
                self.mutantsPerLine[mutation.lineNumber] = 1 + \
    ...
    def aggregateReport(self, littleDarwinVersion: str):
        ...
        output += self.cssStyle + \
            "</style></head><body><h1>LittleDarwin Aggregate Mutation Report</h1>"
        output += "<p>Average Density: {:.2f}".format(
            self.averageDensity) + "</p><div><pre class=\"code\">"
        ...
        for mutant in self.mutants:
            ...
            for mutation in mutant.mutationList:
                mutationStartDict[mutation.startPos] = (
                    mutation.mutatorType, str(mutation))
        ...
        output += "Report generated by LittleDarwin {} </p></footer></body></html>".format(
            littleDarwinVersion)
