...
from mediumdarwin.original import License
from mediumdarwin.original.JavaIO import JavaIO
from mediumdarwin.original.JavaMutate import JavaMutate
# LittleDarwin modules
from mediumdarwin.original.JavaParse import JavaParse
from mediumdarwin.original.ReportGenerator import ReportGenerator
...
def mutationPhase(options, filterType, filterList, higherOrder):
    ...
    for srcFile in javaIO.fileList:
        print("\n(" + str(fileCounter + 1) + "/" +
              str(fileCount) + ") Source file: ", srcFile)
        targetList = list()
        ...
        javaMutate = JavaMutate(
            tree, sourceCode, javaParse, options.isVerboseActive)

        if higherOrder == 1:
            mutated, mutantTypes = javaMutate.gatherMutants(enabledMutators)
        else:
            mutated, mutantTypes = javaMutate.gatherHigherOrderMutants(
                higherOrder, enabledMutators)
        ...
        for mutantType in mutantTypes.keys():
            ...
            mutantTypeDatabase[mutantType] = mutantTypes[mutantType] + \
        ...
        aggregateComplexity = javaIO.getAggregateComplexityReport(javaMutate.mutantsPerMethod,
                                                                  javaParse.getCyclomaticComplexityAllMethods(
                                                                      tree),
...
def buildPhase(options):
    ...
    if options.alternateDb == "***dummy***":
        databasePath = os.path.abspath(os.path.join(
    ...
    try:
        ...
        print("Initial build failed. Try building the system manually first to make sure it can be built. " +
              "Take a look at " +
              os.path.abspath(os.path.join(mutantsPath, "initialbuild.txt"))
    ...
    for key in databaseKeys:
        ...
        print("(" + str(fileCounter) + "/" +
              str(mutationDatabaseLength) + ") collecting results for ", key)
        ...
        for replacementFileRel in mutationDatabase[key]:
            replacementFile = os.path.abspath(
                os.path.join(mutantsPath, replacementFileRel))
            mutantCounter += 1
            ...
            shutil.copyfile(replacementFile, os.path.join(
                options.sourcePath, key))
    ...
    with open(targetHTMLReportFile, 'w', encoding="utf-8") as htmlReportFile:
        htmlReportFile.writelines(reportGenerator.generateHTMLFinalReport(
...
def timeoutAlternative(commandString, workingDirectory, timeout, inputData=None):
    ...
    return isKilled, process.returncode, stdout_str
